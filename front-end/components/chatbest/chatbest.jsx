		...
		if (message.trim() !== "") {
			...
			try {
				const response = await axios.put(
					`http://192.168.1.19:5000/chat/users/update/${selectedUser.fullName}`,
					newMessage,
				);
	...
	const fetchMessages = async () => {
		try {
			...
			const selectedUserMessagesResponse = await axios.get(
				`http://192.168.1.19:5000/chat/users/user/${selectedUser.fullName}`,
				{
			...
			const currentUserMessagesResponse = await axios.get(
				`http://192.168.1.19:5000/chat/users/user/${Currentuser.fullName}`,
				{
